    ...
    <>
      ...
      {annotationMode && (
        <div className="rounded-xl border border-brand-200 bg-brand-50/70 px-3 py-2 text-[11px] leading-relaxed text-brand-900 dark:border-brand-900 dark:bg-brand-950/25 dark:text-brand-100">
          <strong>Annotation mode is active.</strong> Each widget is now a visible target. Use its Annotate button to queue a scoped note; nothing changes until you apply through the selected connected API or import a validated harness result.
        </div>
      )}
      <ViewGrid
        layout={view.layout}
