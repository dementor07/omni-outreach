...
import { Users, Activity, Building2, Flame, ExternalLink, CornerDownRight } from 'lucide-react'
...
export default function Leads() {
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              <thead className="border-b border-slate-200 text-xs uppercase tracking-wider text-slate-500 dark:border-slate-800">
                <tr>
                  {hasScores && <th className="px-4 py-2.5 font-medium">AI</th>}
                  {columns.map((c) => (
                    <th key={c.key} className="px-4 py-2.5 font-medium">{c.key === 'stage' ? 'Step' : c.label}</th>
                  ))}
                </tr>
              </thead>
      ...
      {openLeadId && (
        <LeadDrawer leadId={openLeadId} onClose={() => setOpenLeadId(null)} onSelectLead={setOpenLeadId} />
      )}
    </div>
  )
}
...
function LeadRow({ lead, columns, score, showScore, onOpen }: LeadRowProps) {
  return (
    <tr onClick={onOpen} className="cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-900/50">
      {showScore && (
...
function LeadCell({ column, lead }: { column: LeadColumn; lead: Lead }) {
  ...
  if (column.key === 'identity') {
    const parentCompany = leadParentCompany(lead)
    return (
      <td className="px-4 py-2.5">
        <p className="font-medium text-slate-900 dark:text-white">{lead.identity}</p>
        {parentCompany ? (
          <p className="flex items-center gap-1 text-xs text-slate-400">
            <CornerDownRight size={11} className="shrink-0" /> {parentCompany}
          </p>
        ) : (
...
// For a person-stage lead (a fan-out child under a company), surface the parent
// company so 4 "Sayantan / Cisco" rows read as people *under* Cisco, not dupes.
// Returns null for company-stage leads (their identity already IS the company).
function leadParentCompany(lead: Lead): string | null {
  if (lead.stage !== 'person' && lead.stage !== 'verifying') return null
  const cf = lead.custom_fields as Record<string, unknown>
  const resolution = cf.company_resolution as { name?: string } | undefined
  const item = cf.item as { company_name?: string } | undefined
  const company = resolution?.name || item?.company_name
  return company && company !== lead.identity ? String(company) : null
}
