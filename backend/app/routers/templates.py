...
async def upsert_template(body: TemplateUpsert, user_id: str = Depends(get_current_user)):
    node = await fetch_one(
        "SELECT campaign_id, node_type FROM sequence_nodes WHERE id=$1", body.node_id
    )
    if not node:
        raise HTTPException(status_code=404, detail="Sequence node not found")

    existing = await fetch_one(
        "SELECT id FROM templates WHERE node_id=$1 LIMIT 1", body.node_id
    )
    ...
    return await fetch_one(
        ...
        RETURNING *
        """,
        node["campaign_id"], body.node_id, name, channel,
