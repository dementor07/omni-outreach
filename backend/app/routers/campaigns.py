...
async def update_campaign(campaign_id: str, body: CampaignUpdate, user_id: str = Depends(get_current_user)):
...
async def clone_campaign(campaign_id: str, user_id: str = Depends(get_current_user)):
    ...
    old_nodes = await fetch_all("SELECT * FROM sequence_nodes WHERE campaign_id=$1", campaign_id)
    id_map: dict[str, str] = {}
    for n in old_nodes:
    ...
    old_edges = await fetch_all("SELECT * FROM sequence_edges WHERE campaign_id=$1", campaign_id)
    for e in old_edges:
