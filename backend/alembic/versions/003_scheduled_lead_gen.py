...
def upgrade() -> None:
    op.execute("ALTER TABLE lead_gen_configs ADD COLUMN IF NOT EXISTS cron_schedule TEXT")
    op.execute("ALTER TABLE lead_gen_configs ADD COLUMN IF NOT EXISTS last_run_at TIMESTAMPTZ")
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_lead_gen_configs_cron "
        "ON lead_gen_configs(cron_schedule) WHERE cron_schedule IS NOT NULL"
    )
    op.execute("ALTER TABLE lead_gen_runs ADD COLUMN IF NOT EXISTS triggered_by TEXT DEFAULT 'manual'")
