...
- **Status:** Accepted (shipped 2026-04-24, cron registration fixed 2026-04-25)
...
- **Decision:**
  ...
  3. Implement a worker cron job `cron_reply_intent_timeout` registered at `minute={0, 30}` (i.e. every 30 minutes). The job scans for leads parked at `condition_reply_intent` whose latest `queue.sent_at` for the lead is older than `timeout_days` and routes them via the `timeout` handle.
