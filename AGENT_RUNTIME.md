...
## What I'd want from you before implementing

Two decisions:

1. **Pick the substrate.** Recommended: Option C. If you have a strong
   preference for LangGraph (vendor lock-in is a feature for some teams —
   easier hiring, easier resume bullet) say so.

2. **Pick the v1 toolset.** I propose starting with: `send_linkedin_dm`,
   `send_email`, `mark_hot`, `add_tag`, `end_sequence`. Five tools is enough
   to demonstrate the pattern without bloat. We add more as concrete campaigns
   need them. Veto or expand.

After those two, the work is mechanical — ~1 day, low risk, fully isolated from
the SOTA cut-over.
